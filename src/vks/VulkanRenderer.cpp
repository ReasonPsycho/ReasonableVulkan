...
VkResult VulkanRenderer::createInstance()
{
	...
	if (settings.validation) {
		vks::base::debug::setupDebugingMessengerCreateInfo(debugUtilsMessengerCI);
	...
	if (std::find(supportedInstanceExtensions.begin(), supportedInstanceExtensions.end(), VK_EXT_DEBUG_UTILS_EXTENSION_NAME) != supportedInstanceExtensions.end()) {
		vks::base::debugutils::setup(instance);
	}
...
void VulkanRenderer::createCommandBuffers()
{
	...
	VkCommandBufferAllocateInfo cmdBufAllocateInfo = vks::base::initializers::commandBufferAllocateInfo(cmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, static_cast<uint32_t>(drawCmdBuffers.size()));
...
VkPipelineShaderStageCreateInfo VulkanRenderer::loadShader(std::string fileName, VkShaderStageFlagBits stage)
{
	...
	shaderStage.module = vks::base::tools::loadShader(androidApp->activity->assetManager, fileName.c_str(), *device);
#else
	shaderStage.module = vks::base::tools::loadShader(fileName.c_str(), *device);
#endif
...
void VulkanRenderer::drawUI(const VkCommandBuffer commandBuffer)
{
	if (settings.overlay && ui.visible) {
		const VkViewport viewport = vks::base::initializers::viewport((float)width, (float)height, 0.0f, 1.0f);
		const VkRect2D scissor = vks::base::initializers::rect2D(width, height, 0, 0);
...
void VulkanRenderer::setupDescriptors() {
	...
	std::vector<VkDescriptorPoolSize> poolSizes = {
		vks::base::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 2),
		vks::base::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1)
	};
	VkDescriptorPoolCreateInfo descriptorPoolInfo = vks::base::initializers::descriptorPoolCreateInfo(poolSizes, 2);
	VK_CHECK_RESULT(vkCreateDescriptorPool(*device, &descriptorPoolInfo, nullptr, &descriptorPool));

	// Layout
	std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
		// Binding 0 : Vertex shader uniform buffer
		vks::base::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT, 0),
		// Binding 1 : Fragment shader color map image sampler
		vks::base::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 1)
	};
	VkDescriptorSetLayoutCreateInfo descriptorLayout = vks::base::initializers::descriptorSetLayoutCreateInfo(setLayoutBindings);
	VK_CHECK_RESULT(vkCreateDescriptorSetLayout(*device, &descriptorLayout, nullptr, &descriptorSetLayout));

	// Set
	VkDescriptorSetAllocateInfo allocInfo = vks::base::initializers::descriptorSetAllocateInfo(descriptorPool, &descriptorSetLayout, 1);
	VK_CHECK_RESULT(vkAllocateDescriptorSets(*device, &allocInfo, &descriptorSet));


	//TODO add skybox prob from camera
	std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
		// Binding 0 : Vertex shader uniform buffer
		vks::base::initializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &uniformBuffer.descriptor),
		// Binding 1 : Fragment shader image sampler
		vks::base::initializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &skybox.descriptor)
	};
...
void VulkanRenderer::preparePipelines() {
	// Layout
		VkPipelineLayoutCreateInfo pipelineLayoutCreateInfo = vks::base::initializers::pipelineLayoutCreateInfo(&descriptorSetLayout, 1);
...
*void VulkanExampleBase::loadAssets() {
	...
	for (auto i = 0; i < modelFiles.size(); i++) {
		...
		model.glTF->loadFromFile(getAssetPath() + "models/" + modelFiles[i], vulkanDevice, graphicsQueue, glTFLoadingFlags);
		demoModels.push_back(model);
	}
	// Textures
	skybox.loadFromFile(getAssetPath() + "textures/cubemap_vulkan.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, graphicsQueue);
...
void VulkanRenderer::updateUniformBuffers() { //TODO add light prob by component
...
void VulkanRenderer::buildCommandBuffers() {
	VkCommandBufferBeginInfo cmdBufInfo = vks::base::initializers::commandBufferBeginInfo();

	VkClearValue clearValues[2];
	clearValues[0].color = defaultClearColor;
	clearValues[1].depthStencil = { 1.0f, 0 };

	VkRenderPassBeginInfo renderPassBeginInfo = vks::base::initializers::renderPassBeginInfo();
	...
	for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
	{
		...
		VkViewport viewport = vks::base::initializers::viewport((float)width, (float)height, 0.0f, 1.0f);
		vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);

		VkRect2D scissor = vks::base::initializers::rect2D(width, height, 0, 0);
		vkCmdSetScissor(drawCmdBuffers[i], 0, 1, &scissor);

		vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSet, 0, nullptr);

		//TODO add models from render system
...
VulkanRenderer::VulkanRenderer()
{
	...
	// Configure MoltenVK to use to use a dedicated compute graphicsQueue (see compute[*] and timelinesemaphore samples)
	...
	bool libLoaded = vks::base::android::loadVulkanLibrary();
	assert(libLoaded);
#elif defined(_DIRECT2DISPLAY)
...
VulkanRenderer::~VulkanRenderer()
{
	...
	if (settings.validation)
	{
		vks::base::debug::freeDebugCallback(instance);
	}
	...
	if (*device) {
		...
		skybox.destroy(); //TODO clean skybox and components
	}
}
...
bool VulkanRenderer::initVulkan()
{
	...
	if (result != VK_SUCCESS) {
		vks::base::tools::exitFatal("Could not create Vulkan instance : \n" + vks::base::tools::errorString(result), result);
		return false;
	}

#if defined(VK_USE_PLATFORM_ANDROID_KHR)
	vks::base::android::loadVulkanFunctions(instance);
#endif

	// If requested, we enable the default validation layers for debugging
	if (settings.validation)
	{
		vks::base::debug::setupDebugging(instance);
	}
	...
	if (gpuCount == 0) {
		vks::base::tools::exitFatal("No device with Vulkan support found", -1);
		return false;
	}
	// Enumerate devices
	...
	if (result != VK_SUCCESS) {
		vks::base::tools::exitFatal("Could not enumerate physical devices : \n" + vks::base::tools::errorString(result), result);
		return false;
	}
	...
	vulkanDevice = new vks::base::VulkanDevice(physicalDevice);
	...
	if (result != VK_SUCCESS) {
		vks::base::tools::exitFatal("Could not create Vulkan device: \n" + vks::base::tools::errorString(result), result);
		return false;
	}

	// Get a graphics graphicsQueue from the device
	...
	if (requiresStencil) {
		validFormat = vks::base::tools::getSupportedDepthStencilFormat(physicalDevice, &depthFormat);
	} else {
		validFormat = vks::base::tools::getSupportedDepthFormat(physicalDevice, &depthFormat);
	}
	assert(validFormat);
	...
	VkSemaphoreCreateInfo semaphoreCreateInfo = vks::base::initializers::semaphoreCreateInfo();
	// Create a semaphore used to synchronize image presentation
	// Ensures that the image is displayed before we start submitting new commands to the graphicsQueue
	...
	submitInfo = vks::base::initializers::submitInfo();
...
HWND VulkanRenderer::setupWindow(HINSTANCE hinstance, WNDPROC wndproc, HWND existingWindow) {
			...
			case AINPUT_SOURCE_TOUCHSCREEN: {
				...
				switch (action) {
					case AMOTION_EVENT_ACTION_UP: {
						...
						if (eventTime - downTime <= vks::base::android::TAP_TIMEOUT) {
							float deadZone = (160.f / vks::base::android::screenDensity) * vks::base::android::TAP_SLOP * vks::base::android::TAP_SLOP;
					...
					case AMOTION_EVENT_ACTION_DOWN: {
						// Detect double tap
						int64_t eventTime = AMotionEvent_getEventTime(event);
						if (eventTime - vulkanExample->lastTapTime <= vks::base::android::DOUBLE_TAP_TIMEOUT) {
							float deadZone = (160.f / vks::base::android::screenDensity) * vks::base::android::DOUBLE_TAP_SLOP * vks::base::android::DOUBLE_TAP_SLOP;
							float x = AMotionEvent_getX(event, 0) - vulkanExample->touchPos.x;
							float y = AMotionEvent_getY(event, 0) - vulkanExample->touchPos.y;
							if ((x * x + y * y) < deadZone) {
...
// SRS - Dispatch rendering loop onto a graphicsQueue for max frame rate concurrent rendering vs displayLink vsync rendering
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
	...
	// SRS - When benchmarking, set up termination notification on main thread when concurrent graphicsQueue completes
...
// SRS - Tell rendering loop to quit, then wait for concurrent graphicsQueue to terminate before deleting vulkanExample
...
void VulkanRenderer::createSynchronizationPrimitives()
{
	...
	VkFenceCreateInfo fenceCreateInfo = vks::base::initializers::fenceCreateInfo(VK_FENCE_CREATE_SIGNALED_BIT);
...
void VulkanRenderer::OnUpdateUIOverlay(vks::base::UIOverlay *overlay) {}