...
VkSurfaceFormatKHR SwapChainManager::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
    // First, look for R8G8B8A8_UNORM format as it's a common and well-supported format
    ...
    // Fallback to B8G8R8A8_SRGB if preferred UNORM is not available
    for (const auto& availableFormat : availableFormats) {
    ...
    // If we can't find our preferred formats, just return the first available one
