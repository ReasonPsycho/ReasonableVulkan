...
#endif
{
	...
	if (!fpCreateHeadlessSurfaceEXT){
		vks::base::tools::exitFatal("Could not fetch function pointer for the headless extension!", -1);
	}
	...
	if (err != VK_SUCCESS) {
		vks::base::tools::exitFatal("Could not create surface!", err);
	}
	...
	{
		vks::base::tools::exitFatal("Could not find a graphics and/or presenting queue!", -1);
	}

	if (graphicsQueueNodeIndex != presentQueueNodeIndex) 
	{
		vks::base::tools::exitFatal("Separate graphics and presenting queues are not supported yet!", -1);
	}
...
void VulkanSwapChain::createDirect2DisplaySurface(uint32_t width, uint32_t height)
{
	...
	if(!foundMode)
	{
		vks::base::tools::exitFatal("Can't find a display and a display mode!", -1);
		return;
	}
	...
	if(bestPlaneIndex == UINT32_MAX)
	{
		vks::base::tools::exitFatal("Can't find a plane for displaying!", -1);
		return;
	}
	...
	if (result !=VK_SUCCESS) {
		vks::base::tools::exitFatal("Failed to create surface!", result);
	}
