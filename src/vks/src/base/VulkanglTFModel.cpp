...
void vkglTF::Texture::fromglTfImage(tinygltf::Image &gltfimage, std::string path, vks::base::VulkanDevice *device,
                                    VkQueue copyQueue) {
	this->device = device;
	...
	if (!isKtx) {
		...
		vks::base::tools::setImageLayout(copyCmd, image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
		                           subresourceRange);
		vkCmdCopyBufferToImage(copyCmd, stagingBuffer, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
		                       static_cast<uint32_t>(bufferCopyRegions.size()), bufferCopyRegions.data());
		vks::base::tools::setImageLayout(copyCmd, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
/*
	glTF mesh
*/
vkglTF::Mesh::Mesh(vks::base::VulkanDevice *device, glm::mat4 matrix) {
	this->device = device;
...
void vkglTF::Model::createEmptyTexture(VkQueue transferQueue) {
	...
	VkCommandBuffer copyCmd = device->createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
	vks::base::tools::setImageLayout(copyCmd, emptyTexture.image, VK_IMAGE_LAYOUT_UNDEFINED,
	                           VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, subresourceRange);
	vkCmdCopyBufferToImage(copyCmd, stagingBuffer, emptyTexture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
	                       &bufferCopyRegion);
	vks::base::tools::setImageLayout(copyCmd, emptyTexture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
	                           VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, subresourceRange);
...
void vkglTF::Model::loadImages(tinygltf::Model &gltfModel, vks::base::VulkanDevice *device, VkQueue transferQueue) {
...
void vkglTF::Model::loadFromFile(std::string filename, vks::base::VulkanDevice *device, VkQueue transferQueue,
	...
	if (fileLoaded) {
		...
		vks::base::tools::exitFatal("Could not load glTF file \"" + filename + "\": " + error, -1);
		return;
	}
